...
const BroadcastStrip = ({ noSticky = false }) => {
  ...
  return data?.broadcastV2?.broadcaststripVisible ? (
    <div className=" z-50 fixed top-0 bg-[#2F3640] text-sm text-white flex justify-center h-10 w-full overflow-hidden my-0">
      {data?.broadcastV2?.link !== '' && !!data?.broadcastV2?.link ? (
        <Link href={data?.broadcastV2.link} as={data?.broadcastV2.link}>
          <a href={data?.broadcastV2.link}>
            <div className=" text-center text-xs sm:text-sm my-auto mx-2">
